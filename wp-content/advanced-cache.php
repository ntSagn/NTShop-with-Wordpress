...
if (false === $plugin_location) {
	...
	if (strcasecmp('https://tkthinh.id.vn/', $protocol . '://' . $host . $request_uri) === 0) {
...
if (!@file_exists(WPO_CACHE_CONFIG_DIR . '/config-tkthinh.id.vn.php')) { return; }

$GLOBALS['wpo_cache_config'] = @json_decode(file_get_contents(WPO_CACHE_CONFIG_DIR . '/config-tkthinh.id.vn.php'), true);

if (empty($GLOBALS['wpo_cache_config'])) {
	include_once(WPO_CACHE_CONFIG_DIR . '/config-tkthinh.id.vn.php');
}
